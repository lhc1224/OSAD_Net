...
class ImageDataTrain(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        txt_path=self.txt_path+names[1]
        txts_list=os.listdir(txt_path)
        index=random.randint(0,len(txts_list)-1)
        pose_path=os.path.join(txt_path,txts_list[index])

        ref_image=pose_path.replace(".txt",".jpg")
        ...
        sample = {'image': input_images, 'label': labels,
                  ...
                  "pose_data":pose_data,"ref_name":ref_image}
        return sample
...
class ImageDataTest(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        pose_path=json_path.replace("refs","txts")
        pose_path=pose_path.replace("json","txt")
        pose_data=load_pose(txt_path=pose_path,img_path=ref_path)
        ...
        return {'image': images, 'name': names,
                ...
                'obj_mask':obj_mask,'per_mask':per_mask,
                "pose_data":pose_data,'ref_name':ref_path}
